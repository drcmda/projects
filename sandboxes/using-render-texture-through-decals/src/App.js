...
// Un-comment the debug prop to see the intersecting box!

export const App = () => (
  <Canvas shadows camera={{ position: [-5, 2.5, 10], fov: 20 }}>
    <ambientLight intensity={0.25} />
    <spotLight position={[10, 10, 10]} angle={0.15} penumbra={1} />
    <pointLight position={[-10, -5, -10]} />
    <group position={[0, -0.75, 0]}>
      <Bun />
      <Dodecahedron position={[-1, 2, 0.5]} scale={0.075} />
      <AccumulativeShadows temporal frames={100} scale={12} alphaTest={0.9} position={[0, 0.04, 0]}>
...
function Bun(props) {
  ...
  return (
    <mesh castShadow receiveShadow geometry={nodes.bunny.geometry} {...props} dispose={null}>
      ...
      <Decal /*debug*/ position={[0, 1.2, 0.75]} rotation={0.3} scale={[0.9, 0.25, 1]}>
      ...
      <Decal /*debug*/ position={[-1.0, 1.75, 0.6]} rotation={-0.7} scale={0.25} map={pmndrs} map-anisotropy={16} />
      <Decal /*debug*/ position={[-1.1, 0.85, 0.52]} rotation={0.75} scale={0.2} map={three} map-anisotropy={16} />
      <Decal /*debug*/ position={[0, 1.3, 0.3]} rotation={0.75} scale={0.3} map={react} map-anisotropy={16} />
    </mesh>
  )
}
...
function Dodecahedron(props) {
  ...
  return (
    <group {...props}>
      <mesh ref={meshRef} scale={clicked ? 2.25 : 1.75} onClick={() => click(!clicked)} onPointerOver={() => hover(true)} onPointerOut={() => hover(false)}>
